...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Ceyhun ERTURK");
MODULE_DESCRIPTION("A kernel module for detection and prevention of Arp attack");
MODULE_VERSION("0.1"); 
        ...
        for (ifa = indev->ifa_list; ifa; ifa = ifa->ifa_next) {
            if (ifa->ifa_address == tip) {
                ...
                if (entry && memcmp(entry->mac, sha, ETH_ALEN) != 0) {
                    printk(KERN_INFO "arpguard:  ARP attack detected on %s from %pM\n", ifa->ifa_label, sha);
                    status = NF_DROP;
        ...
        if (dhcp_is_valid(skb) == 0) {
            ...
            switch (dhcp_packet_type) {
                case DHCP_ACK:{
                    ...
                    printk(KERN_INFO "arpguard:  DHCPACK of %pI4\n", &payload->yiaddr);
                    getnstimeofday(&ts);
                ...
                case DHCP_NAK:{
                    printk(KERN_INFO "arpguard:  DHCPNAK of %pI4\n", &payload->yiaddr);
                    entry = find_dhcp_snooping_entry(payload->yiaddr);
                    if (entry) {
                ...
                case DHCP_RELEASE:{
                    printk(KERN_INFO "arpguard:  DHCPRELEASE of %pI4\n", &payload->ciaddr);
                    delete_dhcp_snooping_entry(payload->ciaddr);
                    break;
                }

                case DHCP_DECLINE:{
                    printk(KERN_INFO "arpguard:  DHCPDECLINE of %pI4\n", &payload->ciaddr);
                    delete_dhcp_snooping_entry(payload->ciaddr);
                    break;
                }
            default:
...
static int arp_is_valid(struct sk_buff* skb, const u16 ar_op, const unsigned char* sha, 
    ...
    if (memcmp(sha, shaddr, ETH_ALEN) != 0) {
        printk(KERN_ERR "arpguard:  the sender MAC address %pM in the message body is NOT identical to the source MAC address in the Ethernet header %pM\n", sha, shaddr);
        return -EHWADDR;
    } 

    if (ipv4_is_multicast(sip)) {
        printk(KERN_ERR "arpguard:  the sender ip address %pI4 is multicast\n", sip);
        return -EIPADDR;
    }

    if (ipv4_is_loopback(sip)) {
        printk(KERN_ERR "arpguard:  the sender ip address %pI4 is loopback\n", sip);
        return -EIPADDR;
    }

    if (ipv4_is_zeronet(sip)) {
        printk(KERN_ERR "arpguard:  the sender ip address %pI4 is zeronet\n", sip);
        return -EIPADDR;
    } 
            
    if (ipv4_is_multicast(tip)) {
        printk(KERN_ERR "arpguard:  the target ip address %pI4 is multicast\n", tip);
        return -EIPADDR;
    }
            
    if (ipv4_is_loopback(tip)) {
        printk(KERN_ERR "arpguard:  the target ip address %pI4 is loopback\n", tip);
        return -EIPADDR;
    }
            
    if (ipv4_is_zeronet(tip)) {
        printk(KERN_ERR "arpguard:  the target ip address %pI4 is zeronet\n", tip);
        return -EIPADDR;
    }

    if (ar_op == ARPOP_REPLY) {
         if (memcmp(tha, dhaddr, ETH_ALEN) != 0) {
            printk(KERN_ERR "arpguard:  the target MAC address %pM in the message body is NOT identical 
...
static int __init arpguard_init(void) {
    ...
    if(dhcp_thread) {
        printk(KERN_INFO"arpguard:  DHCP Thread Created Successfully...\n");
    } else {
        printk(KERN_INFO"arpguard:  Cannot create kthread\n");
    }
    return 0; 
}


static void __exit arpguard_exit(void) {
...
module_init(arpguard_init);
module_exit(arpguard_exit);
